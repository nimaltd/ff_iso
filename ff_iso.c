...
//##############################################################################################################
//##############################################################################################################
void ff_task(void const * argument)
{
  while (1)
  {
    switch (ff_iso.func)
...
FRESULT ff_init(osPriority osPriority_)
{
  ...
  osThreadDef(ffTask, ff_task, osPriority_, 0, 512);
  ffTaskHandle = osThreadCreate(osThread(ffTask), NULL);
  if (ffTaskHandle != NULL && ffMutexID != NULL)
    return FR_OK;
  else
